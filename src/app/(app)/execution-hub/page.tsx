...
const THREADS_ICON = (
  <svg viewBox="0 0 32 32" className="w-7 h-7" fill="none">
    ...
    <path d="M21.2 15.2c-.1-.6-.4-1.1-.7-1.5-.5-.6-1.2-1-2-1.2-.1 0-.3-.1-.4-.1-1-.2-2.1 0-3 .6-.1.1-.2.1-.3.2v-3.7H13v9.9c0 .6.1 1.2.4 1.7.4.8 1.1 1.4 2 1.7.4.1.9.2 1.4.2 1.2 0 2.3-.5 3-1.4.6-.7.9-1.7.9-2.7 0-.6-.1-1.2-.3-1.7h-.2zm-2 3.1c-.3.6-.9 1-1.6 1s-1.3-.3-1.6-.9c-.2-.3-.2-.7-.2-1 0-.4.1-.8.3-1.1.3-.5.8-.9 1.5-.9s1.3.3 1.6.9c.2.3.3.7.3 1.1 0 .7-.1 1.3-.3 1.9z" fill="white" />
  </svg>
);
...
function ThreadsConnectCard() {
  ...
  // Check on mount if already connected (read from Firestore via a status endpoint)
  useEffect(() => {
    const check = async () => {
      try {
        const { authFetch } = await import("@/lib/authFetch");
  ...
  // Handle ?threads_connected=1 or ?threads_error=1 redirect from OAuth callback
  useEffect(() => {
        ...
        {status === "connected" && profile ? (
          <div className="flex items-center gap-3 px-4 py-3 rounded-2xl border border-gray-100 bg-[#FAFAFA]">
            {profile.profilePictureUrl ? (
              ...
              <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center shrink-0 text-[12px] font-bold text-[#62646A]">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1 mt-0.5">
                <CheckCircle2 size={11} className="text-[#32C382]" />
                <p className="text-[11px] text-[#32C382] font-medium">Connected</p>
              </div>
            </div>
            <button
              onClick={handleDisconnect}
              disabled={status !== "connected"}
              className="text-[11px] text-[#9A9A9A] hover:text-red-500 transition-colors disabled:opacity-50"
            >
          ...
          <div className="rounded-2xl border border-gray-100 p-4 space-y-3">
            <p className="text-[12px] text-[#62646A] leading-relaxed">
              Connect your Threads account to generate, schedule, and post directly from here.
            </p>
            <button
              onClick={handleConnect}
              disabled={status === "loading"}
              className={cn(
                "flex items-center gap-2 px-4 py-2 rounded-xl text-[12px] font-semibold transition-all bg-[#151515] text-white hover:bg-[#2a2a2a]",
