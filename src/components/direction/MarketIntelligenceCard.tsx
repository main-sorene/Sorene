        ...
        {status === "complete" && report && isExpanded && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
            ...
            <div className="p-5 space-y-8">
              <ReportSection title="Rising Signals" icon={<TrendingUp size={15} className="text-emerald-500" />}>
                <div className="space-y-4">{report.rising_signals.map((s, i) => <SignalRow key={i} signal={s} variant="rising" />)}</div>
              </ReportSection>
              <ReportSection title="Falling Signals" icon={<TrendingDown size={15} className="text-red-400" />}>
                <div className="space-y-4">{report.falling_signals.map((s, i) => <SignalRow key={i} signal={s} variant="falling" />)}</div>
              </ReportSection>
              ...
              <ReportSection title="On the Horizon" icon={<Clock size={15} className="text-[#9A9A9A]" />}>
                <div className="space-y-4">{report.horizon_signals.map((s, i) => <HorizonRow key={i} signal={s} />)}</div>
              </ReportSection>
  ...
  return (
    <div className="rounded-xl border border-[#e0e7ff] bg-[#f5f3ff] p-3.5 flex flex-col gap-2">
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="text-[11px] text-[#6366f1] font-medium uppercase tracking-wide mb-1">Top Match</p>
          <p className="text-[14px] font-semibold text-[#151515] leading-snug">{opportunity.title}</p>
        </div>
        <div className="shrink-0 flex flex-col items-end gap-1">
          <span className="text-[22px] font-bold text-[#4338ca] leading-none">{opportunity.dna_fit_score}</span>
    ...
    <div className="rounded-xl border border-gray-100 bg-white overflow-hidden shadow-sm">
      <button onClick={onToggle} className="w-full flex items-start gap-3 p-4 text-left hover:bg-gray-50 transition-colors">
        <div className="flex flex-col items-center shrink-0 mt-0.5">
          <span className="text-[18px] font-bold leading-none" style={{ color: scoreColor }}>{score}</span>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-1.5 mt-2">
            ...
            <span className="px-2 py-0.5 rounded-full text-[10px] font-medium" style={{ backgroundColor: c.bg, color: c.text }}>{opportunity.startup_cost} · {opportunity.startup_cost_range}</span>
            <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-gray-100 text-[#62646A]"><Zap size={9} className="inline mr-0.5" />{opportunity.time_to_revenue}</span>
          </div>
        </div>
